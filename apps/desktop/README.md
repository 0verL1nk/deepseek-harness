...
`pnpm run desktop:package` builds an unpacked local application after `pnpm run build`, and `pnpm --filter @deepseek-ai/dsh-desktop run boot-smoke` forks the packaged backend and requires its readiness handshake — the packaging gate that proves the pruned dependency set starts the web host. `pnpm run desktop:make` produces the platform distribution: per-architecture macOS DMG and ZIP, Windows NSIS, and Linux AppImage and DEB. The [Desktop workflow](../../.github/workflows/desktop.yml) builds native artifacts for macOS universal, Windows x64, and Linux x64 from `vX.Y.Z` tags (the desktop application versions independently of the bundled dsh core) and attaches them to the corresponding GitHub Release. It derives the installed application version as standard SemVer `X.Y.Z` (including an optional prerelease suffix) from that tag.

The application's runtime dependency set is the web profile's full composition. electron-builder prunes `node_modules` to the dependency graph it can walk, and harness packages reference each other through `peerDependencies`, so packages reachable only over a peer edge — the vendored `cordis-plugin-group` and the dsh service packages other plugins peer on — — across every platform-gated variant of the composition — must be declared as direct dependencies of this package to survive pruning. The Desktop workflow boots each platform's packaged backend before publishing, so a composition change that outruns the declared set fails the build instead of shipping an uninstallable release.
